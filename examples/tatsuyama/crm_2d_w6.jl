# ============================================================
# CRMシャドウの2D拡張 その3: W=6 シリンダー (クラスター計算, 96量子ビット)
#   MPSが本当に苦しくなる幅で「タダのUHF priorが高価なMPS priorに匹敵」を検証
#
...
function ground_state_cyl(Lx, W, t, U, mu; chi_max=192, nsweeps=10, stagger=0.0)
    N = 2 * Lx * W
...
function solve_uhf(Lx, W, t, U; Nup, Ndn, iters=800, tol=1e-10, stagger=0.0)
    n = Lx * W
...
function validate_u0()
    ...
    E_dmrg, ψ = ground_state_cyl(Lxv, W, t, 0.0, 0.0; chi_max=400, nsweeps=40, stagger=stg)
...
function main()
    ...
    rows = []
    uenv = get(ENV, "UVAL", "")
    U_list = uenv == "" ? [4.0, 8.0] : [parse(Float64, uenv)]
    for U in U_list
        println("\n", "="^70)
    ...
    out = joinpath(@__DIR__, "crm_2d_w6_results_U$(utag).tsv")
    open(out, "w") do io
...
println("done (W=$W, LX=$LX, CHI_EXP=$CHI_EXP)")
