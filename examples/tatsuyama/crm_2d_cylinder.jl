# ============================================================
# CRMシャドウの2D拡張 (W=4 ハバードシリンダー, UHF prior vs 切断MPS prior)
#
# 問い:
#  (1) 「局所利得は prior の局所誤差 Δ だけで決まる」法則は2Dでも成立するか
#  (2) 2Dで安価な prior (UHF平均場, O(N³)) は χ制限MPS prior にどこまで匹敵するか
#
# 設計:
...
function main()
    t = 1.0
    chi_priors = [8, 16, 32, 64]
    prior_labels = vcat("UHF", ["chi=$c" for c in chi_priors])
    ...
    for U in [4.0, 8.0]
        ...
        @printf("  MPS prior fidelities: %s\n",
                join([@sprintf("chi=%d: %.4f", c, f) for (c, f) in zip(chi_priors, fids[2:end])], ",  "))
        flush(stdout)
    ...
    out = joinpath(@__DIR__, "crm_2d_results.tsv")
    open(out, "w") do io
...
function gain_panel(rows, U, prior_labels)
    p = plot(xlabel="prior", ylabel="G", yscale=:log10, legend=:outertopright,
             title=@sprintf("W=4 cylinder, U=%.0f", U),
             xticks=(1:length(prior_labels), prior_labels), xrotation=30)
    for name in unique(r[3] for r in rows if r[1] == U)
        g = [only([r[8] for r in rows if r[1]==U && r[2]==lab && r[3]==name])
             for lab in prior_labels]
        plot!(p, 1:length(prior_labels), g, marker=:circle, lw=2, label=name)
    end
    hline!(p, [1.0], color=:black, ls=:dot, label="")
    return p
end

p1 = gain_panel(rows, 4.0, prior_labels)
p2 = gain_panel(rows, 8.0, prior_labels)
...
p3 = scatter(gx, gy, xscale=:log10, yscale=:log10, xlims=lims, ylims=lims,
             ...
             xlabel="G theory", ylabel="G empirical", title="2D: scaling law check")
plot!(p3, [lims...], [lims...], color=:black, ls=:dash, label="y = x")

final = plot(p1, p2, p3, layout=(1,3), size=(1650, 430), margin=6Plots.mm)
outpng = joinpath(@__DIR__, "crm_2d_cylinder.png")
