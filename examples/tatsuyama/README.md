...
**(7) 唯一の例外**: ドープ鎖のホッピングは χ_p=8 でも $G/G_{\max}$ が L とともに単調に低下(L=8→128 で 0.96, 0.88, 0.79, 0.74, 0.72)。この観測量だけは固定χ priorが L とともに相対的に劣化する。
...
3. **ホッピングの L 依存**: ドープ鎖のホッピングだけが $G/G_{\max}$ の L 依存を示した(0.96→0.72)。JW弦を含む観測量に固有の効果か、単に電流揺らぎの相関長が長いだけかの切り分けが要る。
