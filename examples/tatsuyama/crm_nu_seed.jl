...
function main()
    ...
    seeds   = parse.(Int, split(get(ENV, "SEED_LIST", "0"), ","))
    chi_priors = [2, 4, 8, 32]
    ...
    rows = []
    for nu in nu_list, sd in seeds
        t0 = time()
        es, ec = run_locals(ρw, Nw, ow, Pσ_all, trOσ_all;
                            nu, nm, n_repeat, seed = 424242 + nu + 7919*sd)
        @printf("\n-- n_u=%-5d seed=%d (n_repeat=%d, %.0fs) --\n", nu, sd, n_repeat, time()-t0)
        ...
        for (k,o) in enumerate(obs)
            ...
            for (p,lb) in enumerate(labels)
                push!(rows, (L, nu, sd, nm, n_repeat, o.name, a, lb, Otrue[k],
    ...
    out = joinpath(@__DIR__, "crm_nu_seed_results.tsv")
    open(out,"w") do io
        println(io, "L\tn_u\tseed\tn_m\tn_repeat\tobservable\tabsA\tprior\ttrue\tDelta\tG\tG_theory\tCI_relwidth")
