...
**(a1)(a2)** 1D 鎖(OBC、 $U=8$ 、UHF prior、オンサイト ZZ)を $L=8$ から $16$ へ伸ばしたときの**実数値**。忠実度( $\lesssim1$ )と利得( $\approx190$ )は単位も桁も違い、1つの図に2本の縦軸で重ねると目盛りの合わせ方しだいで見かけの関係が作れてしまうので、**上下2枚に分けてある**。**(a1)** 忠実度(対数軸)。**落ちるのは大域忠実度だけ**で $0.160\to0.0391$ 。台の忠実度は $0.5577$ のままで、 $L$ 全体での変動幅は $6\times10^{-5}$ しかない。**(a2)** 同じ prior の利得(線形軸、0 起点)。 $187.9\to191.1$ で、範囲は 187.9〜192.1(変動 2%)に収まり、ほぼ水平である。
...
**(c)** 利得法則が厳密な単一 Pauli 列の全 1436 点(**このパネルだけ OBC と PBC を合わせた全系**。(a1)(a2)(b) は OBC)。**同じ利得の高さに、大域忠実度が4桁にわたって散らばる**(同じ $G$ の帯で $F$ の 5–95 パーセンタイルが 4.6 桁)。集計を一切せずに、忠実度が利得を決めていないことが見える。
...
上の図を**周期境界(トーラス)だけ**で作り直したものである(PBC の厳密系は 1D $L=8$ – $16$ 、2D 2×4, 2×6, 4×3。単一パウリ点は 560)。

**(a1)(a2)** OBC と同じである(軸も OBC 版と同じで、忠実度は対数軸、利得は 0 起点の線形軸)。 $L=8\to16$ で大域忠実度だけが $0.191\to0.0525$ と落ち(OBC は $0.160\to0.0391$ 。灰色の一点鎖線)、**台の忠実度は $0.5578\to0.5577$ で変動幅 $8\times10^{-5}$ 、利得も $185.8\to189.4$ (範囲 184.2〜189.4、変動 3%)でほぼ動かない**。この文書の中心的な主張は開放端の産物ではない。
...
| 図 | 何を示すか |
|---|---|
| [crm_new_fig_edfid.png](crm_new_fig_edfid.png) | (a1)(a2) 系を伸ばすと大域忠実度だけが崩れる(忠実度と利得を実数値で別々の図に) (b) $U$ で電荷量とスピン量が逆を向く (c) 全点で $F$ が $G$ を決めていない |
| [crm_new_fig_edfid_pbc.png](crm_new_fig_edfid_pbc.png) | 上の PBC 版。(a1)(a2)(c) は OBC と同じ結論、**(b) だけ違い**、(d) がその理由(開放鎖の中央ボンドの偶奇による交替) |
