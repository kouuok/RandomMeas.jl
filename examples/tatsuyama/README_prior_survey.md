...
参照状態を作る側でも同じ壁がある。到達範囲を測ると:

| 系 | サイト数 | $\chi$ | 分散 | 判定 |
|---|---|---|---|---|
| 1D( $W=1$ ) | 64 | 886 | 1.1e−10 | **厳密** |
| 2×16( $W=2$ ) | 32 | 4096 | 1.7e−10 | **厳密** |
| **4×4( $W=4$ )** | **16** | 6144(上限) | 5.8e−6 | **近似** |
| 4×4 トーラス | 16 | 6144(上限) | 7.2e−2 | 近似 |

**$W=4$ では16サイト(32量子ビット)でも厳密化できない。** $W=2$ なら32サイト(64量子ビット)まで届くので、効いているのはサイト数ではなく幅である(面積則 $\chi\sim e^{aW}$)。
