...
この総当たりは**シャドウをサンプリングしていない**。各 prior について厳密な $\langle P\rangle$ と $\Delta=\langle P\rangle_\rho-\langle P\rangle_\sigma$ を求め、利得を法則の閉形式
...
で直接評価している([crm_2d_ed_fid_table.jl](crm_2d_ed_fid_table.jl) の `gain(P, Δ, nA)`)。標準シャドウと CRM の分散はどちらも $1/n_u$ に比例するので、**その比である $G$ では $n_u$ が厳密に約分されて消える**。つまり $n_u$ には決めるべき値が存在しない。

これは式の上だけの話ではなく、 $n_u$ を 25 から 1600 まで64倍振ったサンプリングで実測確認している(下記「 $n_u$ と $n_m$ の影響」結果1)。**$n_u$ が決めるのは推定値の絶対精度( $\mathrm{Var}\propto1/n_u$ )だけである。** 一方 $n_m$ はショットノイズ床 $v_s$ に入るので $G$ の値そのものを変える — ただし prior の順位は変えないことを結果2で確認しており、以下の比較の結論は $n_m=100$ という選択に依存しない。
...
**(c)** 利得法則が厳密な単一 Pauli 列の全 1120 点。**同じ利得の高さに、大域忠実度が4桁にわたって散らばる**。集計を一切せずに、忠実度が利得を決めていないことが見える。
