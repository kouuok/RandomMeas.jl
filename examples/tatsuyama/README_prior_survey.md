...
**1D 鎖と 2D 格子、開放境界と周期境界、 $U=2,4,8,12$ の 72 系について、CRM の prior を総当たりで測った記録。** 参照状態 $\rho$ は厳密な基底状態で、prior $\sigma$ は平均場(UHF)と、参照を $\chi_p$ で切断した MPS である。各点について**利得 $G$ ・大域忠実度・観測量の台に縮約した忠実度**を同時に記録した。全 4424 点。
...
3. **平均場が MPS に勝つのは、MPS が張り詰めている系に限る。** ED で届く小さい系では $\chi_p=32$ がほぼ厳密なので MPS が勝つ。
