...
6. **スピン射影 HF(回した UHF を量子的に重ね合わせる)と低い結合次元の MPS も計算して比べた**(5.3、詳しくは [README_phf_mps.md](README_phf_mps.md))。射影は一重項の相関を足すが、その効果は系の大きさに反比例して消え、 $L=64$ では真の値との差の 11% しか埋まらない。MPS は結合ごとの量子的な相関を持つが、厳密な状態を切り詰めたもの(スピン相関が正確、電荷のゆらぎを捨てる)と、結合次元を制限した DMRG(電荷のゆらぎが正確、スピンの向きを選ぶ)で性質が逆になる。
...
UHF-sym は回した UHF を**確率的に混ぜた**もので、干渉がないので、回転で変わらない量はすべて UHF とまったく同じ値のままになる(3.1)。**新しい相関は1つも生まれない。** 5.2 の「直せない誤差」を直せる prior として、次の2つを計算し、同じ表で比べた。
...
**実用上の結論**: 数十〜数百量子ビットの系では、PHF は CRM の prior として UHF-sym からの改善が小さく、計算の手間に見合わない。 5.2 の「古典的な相関の不足」は結合ごとの量子的な相関を持つ MPS で直せるが、実際に作れる変分 MPS では、その対称性の破れを別に取り除く必要がある。
