...
\begin{equation}
  ...
  \label{eq:heisenberg}
\end{equation}
となる(ハイゼンベルク反強磁性模型)。$\vec S_i\cdot\vec S_j$ は 2 つのスピンが反平行だと $-3/4$、平行だと $+1/4$ を返すので、$J>0$ は\emph{反平行を好む}(反強磁性)。すなわち Mott 絶縁体の基底状態は、$\up\dn\up\dn\cdots$ と交互に並ぶ\emph{Néel 秩序}に近い。本研究で $U=8$(強結合)のとき密度・スピン型の観測量が大きな期待値を持ち、CRM の利得が大きくなるのは、この強い秩序が理由である。
