...
\begin{keybox}
\textbf{系の呼び方について.}以下では「1 次元 $L=16$ 鎖」「2 次元 $W=4$ シリンダー」のような書き方をする。$L$ は鎖のサイト数、$W$ は\emph{シリンダーの幅}(2 次元格子を筒状に丸めたときの円周方向のサイト数)である。$W=4$ なら円周 4 サイト、$W=6$ なら 6 サイトで、$W$ が大きいほど本物の 2 次元平面に近い。なお文献では同じ $W$ を「バンド幅」(エネルギーの尺度)に使うこともあるが、本稿では格子の形のみを指す。
\end{keybox}
...
\begin{equation}
  ...
  \qquad v_s=\frac{3^{|A|}(1-\ev{P}^2)}{n_m}
\end{equation}
となる。記号を整理しておく:
...
記号 & 定義 \\
\midrule
$P$ & 測りたい局所パウリ列。例:$Z_1Z_2$ \\
$A$ & $P$ の\emph{台} --- 恒等演算子でないパウリが載る量子ビットの集合。$|A|$ はその個数 \\
$\rho,\ \sigma$ & 実験状態と prior(古典計算で作った $\rho$ の近似) \\
$\ev{P}$ & 実験状態での期待値 $\Tr[P\rho]$(添字を省いたものは常に $\rho$ 側) \\
$\Delta$ & prior の\emph{局所誤差} $\ev{P}_\rho-\ev{P}_\sigma$ \\
$n_m$ & 1 つの測定設定あたりのショット数 \\
$v_s$ & ショットノイズ床(prior では打ち消せない分散) \\
...
\noindent 設定数 $n_u$ が式に現れないのは、標準・CRM のどちらの分散も $1/n_u$ に比例し、比を取ると約分されるためである。これを観測量の性質と結びつけると、次の 3 つの「効きどころ」が読める。
