...
機構は明快である。強結合では二重占有は完全に仮想的で、ホッピングがダブロン・ホロン対を作る高エネルギー過程だけが担う。これは波動関数の\emph{振幅の小さい成分}であり、Schmidt 分解では低重み側に住む。$\chi_p=8$ の切断はまさにそこを捨てる。定量的には、捨てた重み $w$ のうち $\ev{D}$ の誤差に転化する割合が
\[
  |\Delta_D|/w = 24.4\%\ (U{=}2),\quad 1.0\%\ (U{=}4),\quad 41.5\%\ (U{=}8)
\]
と $40$ 倍変動し、しかも $U$ に対し\emph{非単調}である($w$ 自体は $U$ とともに単調に減る)。

\paragraph{中心主張の拡張。}prior の良し悪しを測る正しい尺度は、グローバル忠実度でも(第\ref{sec:locality}節)、切断で捨てる重みでもなく(本節)、\textbf{観測量が敏感な成分を prior が保っているか}である。局所性の原理を実空間からエンタングルメントスペクトルへ持ち上げたことになる。実務的には「$\chi$ をいくつにすれば十分か」は観測量ごとに違い、\emph{仮想過程が担う量は支配的な相関よりずっと大きな $\chi$ を要求する}。
