...
窓のビット順は b_{q1} が最上位。観測量の台に含まれない量子ビットは Δ に
一切寄与しないので、仮説「局所観測量には部分系の忠実度が効く」を検証するには
**窓を台そのものに絞る**必要がある。"""
function ptrace_window(ρw::AbstractMatrix, Nw::Int, keep::Vector{Int})
    ks = sort(keep); nk = length(ks)
    dk = 1 << nk
    out = zeros(ComplexF64, dk, dk)
    rest = [q for q in 1:Nw if !(q in ks)]
    # 窓の添字 s(0始まり)から、keep 部分と rest 部分のビットを取り出す
    bitof(s, pos) = (s >> (Nw - pos)) & 1          # pos=1 が最上位
    kidx(s) = begin
        v = 0; for (j,q) in enumerate(ks); v |= bitof(s,q) << (nk - j); end; v
    end
    ridx(s) = begin
        v = 0; for (j,q) in enumerate(rest); v |= bitof(s,q) << (length(rest) - j); end; v
    end
...
function main()
        ...
        for i in sites
            ...
            for (lb, σw) in cands
                ...
                for (k,o) in enumerate(obs)
                    FA, DA = isempty(supp[k]) ? (1.0, 0.0) :
                        fid_trdist(Matrix(ptrace_window(ρw, Nw, supp[k])),
                                   Matrix(ptrace_window(σw, Nw, supp[k])))
                    ...
                    push!(rows, (L, δ, i, o.name, o.pure, absA, lb, Otrue[k], Δ, abs(Δ),
                                 Fw, 1-Fw, Dtr, FA, 1-FA, DA, length(supp[k]), gth, gmax))
                end
            end
        end
    ...
    out = joinpath(@__DIR__, "crm_support_fidelity_results.tsv")
    open(out,"w") do io
        println(io, "L\tdoping\tsite\tobservable\tpure\tabsA\tprior\ttrue\tDelta\tabsDelta\t" *
                    "F_window\tinfid\tD_trace\tF_supp\tinfid_supp\tD_supp\tnsupp\tG_theory\tG_max")
