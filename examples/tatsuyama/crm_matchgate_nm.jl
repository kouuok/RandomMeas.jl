# ============================================================
# matchgateシャドウ × CRM: ショット配分 nm 掃引 (+最適β)
#   §3.5の開いた問い「CRMはmatchgateにも nm 大なら効くのか」を検証する。
#   Gauss回転アンサンブルの設定間揺らぎ Var_u(E[X|u]) は小さいが非零なので、
#   ショットノイズが 1/nm で消える大nm極限では CRM利得が現れるはず。
#   最適β版 (制御変量係数をデータから推定) も同時に評価する。
# 元: crm_matchgate.jl
#
...
#
# 実行方法:
#  JULIA_LOAD_PATH="@:@v#.#:@stdlib" julia --project=Hubbard_MPS_Env_v2 crm_matchgate_nm.jl
# ============================================================
...
function main()
    ...
    for U in [4.0]
        ...
        # --- 実験ループ (matchgateのみ, nm掃引; std / CRM / opt-β) ---
        nm_list = [30, 100, 300, 1000, 3000]
        for nm_ in nm_list
            n_rep = nm_ >= 1000 ? 25 : 40
            est = zeros(n_rep, nobs, 3)
            mMu = zeros(nu, nobs); Yu = zeros(nu, nobs)
            b = zeros(Int, NQ)
            tstart = time()
            for rep in 1:n_rep
                for iu in 1:nu
                    ...
                    Kp = transpose(Q) * K * Q
                    for k in 1:nobs; mMu[iu, k] = 0.0; end
                    for _ in 1:nm_
                        s_ = searchsortedfirst(cp, rand()) - 1
                        for q in 1:NQ; b[q] = bitof(s_, q, NQ); end
                        for k in 1:nobs; mMu[iu, k] += eval_obs(oes[k], b); end
                    end
                    for k in 1:nobs
                        mMu[iu, k] /= nm_
                        Yu[iu, k] = exact_obs_mean(oes[k], Kp)
                    end
                end
                for k in 1:nobs
                    x = view(mMu, :, k); y = view(Yu, :, k)
                    mx = mean(x); my = mean(y)
                    est[rep, k, 1] = mx
                    est[rep, k, 2] = mx - my + trOσ_m[k]
                    vy = var(y)
                    β = vy > 1e-12 ? cov(x, y) / vy : 0.0
                    est[rep, k, 3] = mx - β * (my - trOσ_m[k])
                end
            end
            @printf("\n  --- nm=%d (n_rep=%d, %.0fs) ---\n", nm_, n_rep, time()-tstart)
            @printf("  %-12s %9s | %-19s %-19s %-19s | %6s %6s\n",
                    "observable", "true", "MG std", "MG CRM(UHF)", "MG opt-beta", "G_CRM", "G_opt")
            for k in 1:nobs
                m = [mean(est[:,k,e]) for e in 1:3]; s = [std(est[:,k,e]) for e in 1:3]
                @printf("  %-12s %9.4f | %8.4f ± %-8.4f %8.4f ± %-8.4f %8.4f ± %-8.4f | %6.2f %6.2f\n",
                        mob[k].name, truth[k], m[1], s[1], m[2], s[2], m[3], s[3],
                        (s[2] > 0 ? (s[1]/s[2])^2 : NaN), (s[3] > 0 ? (s[1]/s[3])^2 : NaN))
                push!(rows, (nm_, mob[k].name, truth[k], s[1], s[2], s[3]))
            end
            flush(stdout)
        end
    end

    out = joinpath(@__DIR__, "crm_matchgate_nm.tsv")
    open(out, "w") do io
        println(io, "nm\tobservable\ttrue\terr_std\terr_crm\terr_opt")
