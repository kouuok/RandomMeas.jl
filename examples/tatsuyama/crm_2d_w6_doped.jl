# ============================================================
# CRMシャドウの2D拡張 その5: W=6 ドープ系 (96量子ビット, 1/8 hole doping)
#   ドープ+広幅 = DMRGが最も苦しむ領域でのprior比較 (UHFはストライプ解)
#
...
# 1. DMRG参照状態
# ------------------------------------------------------------
# stagger: 副格子 (電荷密度波) ポテンシャル。検証系(U=0)の縮退を持ち上げ、
#   一意なギャップ付き閉殻 (=RHF が厳密) にするために使う。本計算では stagger=0。
function hubbard_cyl_mpo(sites, Lx, W, t, U, mu; stagger=0.0)
    os = OpSum()
...
function ground_state_cyl(Lx, W, t, U, mu; chi_max=192, nsweeps=10, stagger=0.0, nhole=NHOLE)
    N = 2 * Lx * W
    sites = siteinds("Fermion", N; conserve_qns=true)
    H = hubbard_cyl_mpo(sites, Lx, W, t, U, mu; stagger)
    init = fill("Emp", N)
    if stagger == 0.0
        # Néel初期状態 + nhole個のホールをxに分散して抜く
        ups = Int[]; dns = Int[]
        for x in 1:Lx, y in 1:W
            s = sidx(x, y)
            q = iseven(x + y) ? qup(s) : qdn(s)
            init[q] = "Occ"
            isodd(q) ? push!(ups, q) : push!(dns, q)
        end
        if nhole > 0
            for k in 1:(nhole ÷ 2)
        ...
        # CDW初期状態: 優先副格子((x+y)奇, ポテンシャル低)を二重占有
        for x in 1:Lx, y in 1:W
            if isodd(x + y)
                s = sidx(x, y); init[qup(s)] = "Occ"; init[qdn(s)] = "Occ"
            end
        end
    end
    ψ0 = productMPS(sites, init)
    ramp = [20, 50, 100, 200, 300, 400, chi_max]
    maxdims = min.(chi_max, [ramp; fill(chi_max, max(nsweeps - length(ramp), 0))])[1:nsweeps]
    nz = [1e-5, 1e-5, 1e-6, 1e-6, 1e-7, 1e-8, 1e-9, 1e-10]
...
function solve_uhf(Lx, W, t, U; Nup, Ndn, iters=2000, tol=1e-10, stagger=0.0, init=:neel)
    n = Lx * W
    T = hopping_matrix(Lx, W, t)
    for x in 1:Lx, y in 1:W          # 副格子ポテンシャル (検証用; 本計算は0)
        T[sidx(x, y), sidx(x, y)] += stagger * (-1.0)^(x + y)
    end
...
function build_observables_2d(Lx, W)
    ...
    for st in (s0, sx1)   # サイト分解密度 (電荷変調の直接測定)
...
function validate_u0()
    t = 1.0
    Lxv = 2
    # W=6 等では U=0 half-filling でフェルミ準位が縮退し基底状態が非一意になる。
    # 副格子ポテンシャル stg で縮退を持ち上げ、一意なギャップ付き閉殻(=RHF厳密)にする。
    # 強い副格子場でギャップを大きく開ける (RHF厳密・DMRG収束良)。
    # CDW自由フェルミオンのDMRGは収束が遅い (診断: 40sweep/chi400でdiff 7e-5)
    # ため、検証は「符号バグならO(0.1)ずれる」ことの検出が目的で許容5e-4とする。
    stg = parse(Float64, get(ENV, "STG", "5.0"))
    n = Lxv * W; Nup = n ÷ 2; Ndn = n ÷ 2
    T = hopping_matrix(Lxv, W, t)
    for x in 1:Lxv, y in 1:W
        T[sidx(x, y), sidx(x, y)] += stg * (-1.0)^(x + y)
    end
    ...
    E_dmrg, ψ = ground_state_cyl(Lxv, W, t, 0.0, 0.0; chi_max=400, nsweeps=40, stagger=stg, nhole=0)
    BLAS.set_num_threads(1)
    @printf("  [check] E_DMRG(U=0) = %.8f  (diff %.2e)\n", E_dmrg, abs(E_dmrg - E_free))
    @assert abs(E_dmrg - E_free) < 5e-4
...
function main()
    ...
    n_repeat = parse(Int, get(ENV, "NREP", "30"))
    n = LX * W
    Nel = n - NHOLE
    ...
    rows = []
    uenv = get(ENV, "UVAL", "8.0")
    U_list = [parse(Float64, uenv)]
    for U in U_list
        println("\n", "="^70)
        @printf("W=%d x Lx=%d cylinder (N=%d qubits), U=%.1f, doping=%d holes (Nel=%d)\n", W, LX, 2n, U, NHOLE, n - NHOLE)
        flush(stdout)
...
# プロットはローカルで crm_2d_w6_results.tsv から生成する (ヘッドレスクラスタのため省略)
println("done (W=$W doped NHOLE=$NHOLE, CHI_EXP=$CHI_EXP)")
